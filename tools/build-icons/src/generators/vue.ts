...
const templatesRoot = path.resolve(here, '..', 'templates', 'vue');
...
const renderYearSfc = (input: IconInput, year: string): { content: string; file: string } => {
  ...
  if (typeof rendered !== 'string') {
    throw new Error(`[${input.slug}/${year}] vue icon-year template returned non-string`);
  }
...
/**
 * Generate every file under `packages/vue/src/icons/`: one `.vue` SFC per
 * brand-year (`<Brand><Year>.vue`) plus the barrel `index.ts` which
 * re-exports `<Brand>Latest` aliases pointing at `meta.latest`.
 */
export const generateVue = async (inputs: readonly IconInput[]): Promise<void> => {
  const vueSrc = packageSrc('vue');
  const iconsDir = path.join(vueSrc, 'icons');
  ...
  for (const input of inputs) {
    for (const entry of input.meta.years) {
      ...
      await ensureWrite(path.join(iconsDir, `${file}.vue`), content);
      keepFiles.add(file);
